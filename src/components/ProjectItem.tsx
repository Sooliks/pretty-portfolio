    ...
    return (
        <div className={'flex flex-col p-2'}>
            <div className={'flex flex-row items-start'}>
                ...
                <Input className={'ml-2'} label={'Ссылка на исходный материал'} value={project?.link || ""} onChange={(e)=>setProject({...project, link: e.target.value})}/>
                <Input className={'ml-2'} label={'Ссылка на демонстрационную версию'} value={project?.demo_link || ""} onChange={(e)=>setProject({...project, demo_link: e.target.value})}/>
            </div>
